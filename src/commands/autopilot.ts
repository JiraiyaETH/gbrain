...
export type AutopilotInstallMode = 'daemon' | 'observe-propose';

export function resolveAutopilotInstallMode(args: string[]): AutopilotInstallMode {
  return isAutopilotProposeOnly(args) ? 'observe-propose' : 'daemon';
}
...
export async function runAutopilot(engine: BrainEngine, args: string[]) {
  if (args.includes('--help') || args.includes('-h')) {
    console.log(
      ...
      '--install --observe installs a recurring one-shot observe/propose canary\n' +
      '(launchd StartInterval on macOS; no KeepAlive daemon, no worker spawn).\n\n' +
...
function plistPath(mode: AutopilotInstallMode = 'daemon'): string {
  const label = mode === 'observe-propose' ? 'com.gbrain.autopilot.observe-propose' : 'com.gbrain.autopilot';
...
export interface AutopilotWrapperScriptOptions {
...
export function buildAutopilotWrapperScript(
  gbrainPath: string,
  repoPath: string,
  opts: AutopilotWrapperScriptOptions = {},
): string {
  const mode = opts.mode ?? 'daemon';
  const intervalSeconds = Math.max(1, Math.floor(opts.intervalSeconds ?? 600));
...
# Sources shell profile for API keys, then ${purpose}
...
${runtimeBinExport}${command}
`;
}

function writeWrapperScript(repoPath: string, opts: AutopilotWrapperScriptOptions = {}): string {
  const home = process.env.HOME || '';
  const gbrainDir = join(home, '.gbrain');
  mkdirSync(gbrainDir, { recursive: true });

  const mode = opts.mode ?? 'daemon';
  const wrapperPath = join(gbrainDir, mode === 'observe-propose' ? 'autopilot-observe-propose-run.sh' : 'autopilot-run.sh');
  const wrapper = buildAutopilotWrapperScript(resolveGbrainCliPath(), repoPath, {
    ...opts,
    runtimeBinOverride: process.env.GBRAIN_BIN || opts.runtimeBinOverride,
  });
...
async function installDaemon(engine: BrainEngine, args: string[]) {
  ...
  switch (target) {
    case 'macos':
      installLaunchd(wrapperPath, home, repoPath, { mode: installMode, intervalSeconds });
      break;
    case 'linux-systemd':
...
export function generateLaunchdPlist(
  wrapperPath: string,
  home: string,
  opts: LaunchdPlistOptions = {},
): string {
  const label = opts.label ?? 'com.gbrain.autopilot';
  const keepAlive = opts.keepAlive ?? true;
  const stdoutName = opts.stdoutName ?? 'autopilot.log';
  const stderrName = opts.stderrName ?? 'autopilot.err';
  ...
  -->
  <key>ThrottleInterval</key><integer>${Math.max(1, Math.floor(opts.throttleIntervalSeconds ?? 60))}</integer>`
    : `  <key>StartInterval</key><integer>${Math.max(1, Math.floor(opts.startIntervalSeconds ?? 600))}</integer>`;
...
${lifecycle}
  <key>StandardOutPath</key><string>${escapeXml(home)}/.gbrain/${escapeXml(stdoutName)}</string>
  <key>StandardErrorPath</key><string>${escapeXml(home)}/.gbrain/${escapeXml(stderrName)}</string>
</dict>
</plist>`;
}
...
function installLaunchd(
  ...
  home: string,
  repoPath: string,
  opts: { mode?: AutopilotInstallMode; intervalSeconds?: number } = {},
) {
  const mode = opts.mode ?? 'daemon';
  const label = mode === 'observe-propose' ? 'com.gbrain.autopilot.observe-propose' : 'com.gbrain.autopilot';
  const plistContent = mode === 'observe-propose'
    ? generateLaunchdPlist(wrapperPath, home, {
      label,
      keepAlive: false,
      startIntervalSeconds: opts.intervalSeconds ?? 600,
  ...
  try {
    ...
    const plist = plistPath(mode);
    writeFileSync(plist, plistContent);
    execSync(`launchctl unload "${plist}" 2>/dev/null || true`, { stdio: 'pipe' });
    execSync(`launchctl load "${plist}"`, { stdio: 'pipe' });
    console.log(`Installed launchd service: ${label}`);
    console.log(`  Mode: ${mode}`);
    console.log(`  Repo: ${repoPath}`);
    console.log(`  Log: ~/.gbrain/${mode === 'observe-propose' ? 'autopilot-observe-propose.log' : 'autopilot.log'}`);
...
function uninstallDaemon() {
  ...
  const wrapperPaths = [
    ...
    join(home, '.gbrain', 'autopilot-observe-propose-run.sh'),
  ];
  ...
  // macOS launchd
  for (const mode of ['daemon', 'observe-propose'] as const) {
    const launchdPlist = plistPath(mode);
    const label = mode === 'observe-propose' ? 'com.gbrain.autopilot.observe-propose' : 'com.gbrain.autopilot';
    if (existsSync(launchdPlist)) {
      try {
        execSync(`launchctl unload "${launchdPlist}" 2>/dev/null || true`, { stdio: 'pipe' });
        unlinkSync(launchdPlist);
  ...
  // Wrapper scripts — shared by all targets
