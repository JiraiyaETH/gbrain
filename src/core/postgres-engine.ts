...
export class PostgresEngine implements BrainEngine {
  ...
  async findOrphanPages(opts?: {
    sourceId?: string;
    sourceIds?: string[];
  }): Promise<Array<{ slug: string; title: string; domain: string | null; type: string | null; page_kind: string | null; source_id: string }>> {
    const sql = this.sql;
    ...
    const rows = await sql`
      ...
      ORDER BY p.slug
    `;
    return rows as unknown as Array<{ slug: string; title: string; domain: string | null; type: string | null; page_kind: string | null; source_id: string }>;
  }
