  ...
  private async applyForwardReferenceBootstrap(injectedConn?: postgres.Sql): Promise<void> {
    ...
    const probeRows = await conn<{
      ...
      SELECT
        ...
        EXISTS (SELECT 1 FROM information_schema.columns
                WHERE table_schema = current_schema() AND table_name = 'timeline_entries' AND column_name = 'origin_key') AS timeline_origin_key_exists
    `;
    ...
    const needsTimelineParserProvenance = probeCr.timeline_entries_exists === true
      && (!probeCr.timeline_managed_by_exists || !probeCr.timeline_origin_key_exists);
  ...
  async searchVector(embedding: Float32Array, opts?: SearchOpts): Promise<SearchResult[]> {
    ...
    const rows = await this.withScopedReadTransaction(opts?.sourceIds, opts?.sourceId, async (tx) => {
      ...
      if (useIterativeHnsw) {
        ...
        await tx`SET LOCAL hnsw.ef_search = 1000`;
        await tx`SET LOCAL hnsw.iterative_scan = strict_order`;
        await tx`SET LOCAL hnsw.max_scan_tuples = 100000`;
      }
      return await tx.unsafe(rawQuery, params as Parameters<typeof tx.unsafe>[1]);
    }, { alwaysTransaction: true });
    return rows.map(rowToSearchResult);
