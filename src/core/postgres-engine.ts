...
export class PostgresEngine implements BrainEngine {
  ...
  private activeSourceId(sourceId?: string | null): string {
  ...
  async getPage(slug: string, opts?: { sourceId?: string; includeDeleted?: boolean }): Promise<Page | null> {
    ...
    const sourceCondition = sourceId ? sql`AND source_id = ${this.activeSourceId(sourceId)}` : sql``;
    ...
    const aliasSourceCondition = sourceId ? sql`AND a.source_id = ${this.activeSourceId(sourceId)}` : sql``;
  ...
  async putPage(slug: string, page: PageInput): Promise<Page> {
    ...
    const sourceId = this.activeSourceId(page.source_id);
    const rows = await sql`
  ...
  async deletePage(slug: string, sourceId?: string): Promise<void> {
    ...
    await sql`DELETE FROM pages WHERE slug = ${slug} AND source_id = ${src}`;
  }
  ...
  async listPages(filters?: PageFilters): Promise<Page[]> {
    ...
    const sourceFilter = filters?.sourceId ? this.activeSourceId(filters.sourceId) : null;
    const sourceCondition = sourceFilter ? sql`AND p.source_id = ${sourceFilter}` : sql``;
    ...
    const rows = await sql`
      ...
      ${tagJoin}
      WHERE 1=1 ${typeCondition} ${tagCondition} ${updatedCondition} ${sourceCondition} ${slugCondition} ${deletedCondition}
  ...
  async getAllSlugs(sourceId?: string): Promise<Set<string>> {
    ...
    const sourceCondition = src ? sql`WHERE source_id = ${this.activeSourceId(src)}` : sql``;
    const rows = await sql`SELECT slug FROM pages ${sourceCondition}`;
  ...
  async resolveSlugs(partial: string, opts?: { sourceId?: string }): Promise<string[]> {
    const sql = this.sql;
    const sourceId = opts?.sourceId || process.env.GBRAIN_SOURCE;
    const pageSourceCondition = sourceId ? sql`AND source_id = ${this.activeSourceId(sourceId)}` : sql``;

    // Try exact canonical match first.
    const exact = await sql`
      SELECT slug FROM pages
      WHERE slug = ${partial} ${pageSourceCondition} AND deleted_at IS NULL
      LIMIT 1
    `;
    if (exact.length > 0) return [exact[0].slug as string];

    // Then exact alias match, returning canonical slugs.
    const aliasSourceCondition = sourceId ? sql`AND a.source_id = ${this.activeSourceId(sourceId)}` : sql``;
    const alias = await sql`
      SELECT DISTINCT p.slug
      FROM page_aliases a
      JOIN pages p ON p.id = a.page_id
      WHERE a.alias_slug = ${partial} ${aliasSourceCondition}
    ...
    // Fuzzy match via pg_trgm across canonical and alias slugs.
    const fuzzyPageSourceCondition = sourceId ? sql`AND p.source_id = ${this.activeSourceId(sourceId)}` : sql``;
    const fuzzyAliasSourceCondition = sourceId ? sql`AND a.source_id = ${this.activeSourceId(sourceId)}` : sql``;
    const fuzzy = await sql`
      WITH candidates AS (
        SELECT p.slug, similarity(p.title, ${partial}) AS sim
        FROM pages p
        WHERE p.deleted_at IS NULL
          AND (p.title % ${partial} OR p.slug ILIKE ${'%' + partial + '%'})
          ${fuzzyPageSourceCondition}
        UNION
        SELECT p.slug, similarity(a.alias_slug, ${partial}) AS sim
        FROM page_aliases a
        JOIN pages p ON p.id = a.page_id
        WHERE p.deleted_at IS NULL
          AND (a.alias_slug % ${partial} OR a.alias_slug ILIKE ${'%' + partial + '%'})
          ${fuzzyAliasSourceCondition}
      )
  ...
  async upsertChunks(slug: string, chunks: ChunkInput[], sourceId?: string): Promise<void> {
    ...
    // Get page_id
    const pages = await sql`SELECT id FROM pages WHERE slug = ${slug} AND source_id = ${src}`;
  ...
  async getChunks(slug: string, sourceId?: string): Promise<Chunk[]> {
    ...
    const rows = await sql`
      ...
      JOIN pages p ON p.id = cc.page_id
      WHERE p.id = COALESCE(
        (SELECT id FROM pages WHERE slug = ${slug} AND source_id = ${src} LIMIT 1),
        (SELECT page_id FROM page_aliases WHERE alias_slug = ${slug} AND source_id = ${src} LIMIT 1)
      )
  ...
  async countStaleChunks(sourceId?: string): Promise<number> {
    const sql = this.sql;
    const src = sourceId || process.env.GBRAIN_SOURCE;
    const sourceCondition = src ? sql`AND p.source_id = ${this.activeSourceId(src)}` : sql``;
    const [row] = await sql`
  ...
  async deleteChunks(slug: string, sourceId?: string): Promise<void> {
    ...
    await sql`
      ...
      WHERE page_id = (SELECT id FROM pages WHERE slug = ${slug} AND source_id = ${src})
    `;
  }
  ...
  async addLink(
    ...
    const sql = this.sql;
    const fromSrc = this.activeSourceId(fromSourceId);
    const toSrc = this.activeSourceId(toSourceId);
    const originSrc = this.activeSourceId(originSourceId);
  ...
  async removeLink(from: string, to: string, linkType?: string, linkSource?: string, fromSourceId?: string, toSourceId?: string): Promise<void> {
    ...
    const fromSourceCondition = fromSrc ? sql`AND f.source_id = ${this.activeSourceId(fromSrc)}` : sql``;
    const toSourceCondition = toSrc ? sql`AND t.source_id = ${this.activeSourceId(toSrc)}` : sql``;
    const typeCondition = linkType !== undefined ? sql`AND links.link_type = ${linkType}` : sql``;
    const sourceCondition = linkSource !== undefined ? sql`AND links.link_source IS NOT DISTINCT FROM ${linkSource}` : sql``;
    await sql`
      DELETE FROM links
      USING pages f, pages t
      WHERE links.from_page_id = f.id
        AND links.to_page_id = t.id
        AND f.slug = ${from} ${fromSourceCondition}
        AND t.slug = ${to} ${toSourceCondition}
        ${typeCondition} ${sourceCondition}
    `;
  }
  ...
  async getLinks(slug: string, sourceId?: string): Promise<Link[]> {
    ...
    const pageIdExpr = src
      ? sql`COALESCE(
          (SELECT id FROM pages WHERE slug = ${slug} AND source_id = ${this.activeSourceId(src)} LIMIT 1),
          (SELECT page_id FROM page_aliases WHERE alias_slug = ${slug} AND source_id = ${this.activeSourceId(src)} LIMIT 1)
        )`
      : sql`COALESCE(
          (SELECT id FROM pages WHERE slug = ${slug} LIMIT 1),
          (SELECT page_id FROM page_aliases WHERE alias_slug = ${slug} LIMIT 1)
        )`;
    const rows = await sql`
  ...
  async addTag(slug: string, tag: string, sourceId?: string): Promise<void> {
    ...
    const page = await sql`SELECT id FROM pages WHERE slug = ${slug} AND source_id = ${src}`;
  ...
  async getTimeline(slug: string, opts?: TimelineOpts): Promise<TimelineEntry[]> {
    ...
    const sourceId = opts?.sourceId || process.env.GBRAIN_SOURCE;
    const sourceFilter = sourceId ? this.activeSourceId(sourceId) : null;
    const pageIdExpr = sourceFilter
      ? sql`COALESCE(
          (SELECT id FROM pages WHERE slug = ${slug} AND source_id = ${sourceFilter} LIMIT 1),
          (SELECT page_id FROM page_aliases WHERE alias_slug = ${slug} AND source_id = ${sourceFilter} LIMIT 1)
        )`
      : sql`COALESCE(
          (SELECT id FROM pages WHERE slug = ${slug} LIMIT 1),
          (SELECT page_id FROM page_aliases WHERE alias_slug = ${slug} LIMIT 1)
    ...
    const rows = await sql`
      ...
      WHERE te.page_id = ${pageIdExpr} ${afterFilter} ${beforeFilter}
  ...
  async getRawData(slug: string, source?: string, sourceId?: string): Promise<RawData[]> {
    ...
    const sourceFilter = brainSourceId ? this.activeSourceId(brainSourceId) : null;
    const pageIdExpr = sourceFilter
      ? sql`COALESCE(
          (SELECT id FROM pages WHERE slug = ${slug} AND source_id = ${sourceFilter} LIMIT 1),
          (SELECT page_id FROM page_aliases WHERE alias_slug = ${slug} AND source_id = ${sourceFilter} LIMIT 1)
    ...
    const rawSourceFilter = source ? sql`AND rd.source = ${source}` : sql``;
    const rows = await sql`
      SELECT rd.source, rd.data, rd.fetched_at FROM raw_data rd
      WHERE rd.page_id = ${pageIdExpr} ${rawSourceFilter}
    `;
  ...
  async createVersion(slug: string, sourceId?: string): Promise<PageVersion> {
    ...
    const rows = await sql`
      ...
      FROM pages WHERE slug = ${slug} AND source_id = ${src}
      RETURNING *
    `;
  ...
  async getStats(): Promise<BrainStats> {
    ...
    const sourceCondition = src ? sql`AND p.source_id = ${this.activeSourceId(src)}` : sql``;
    const typeWhere = src ? sql`WHERE p.deleted_at IS NULL AND p.source_id = ${this.activeSourceId(src)}` : sql`WHERE p.deleted_at IS NULL`;
  ...
  async getHealth(): Promise<BrainHealth> {
    const sql = this.sql;
    const src = process.env.GBRAIN_SOURCE;
    const sourceCondition = src ? sql`AND source_id = ${this.activeSourceId(src)}` : sql``;
    ...
    // definition. A hub page that links out to many but has no back-references
  ...
  // Sync
  async updateSlug(oldSlug: string, newSlug: string, sourceId?: string): Promise<void> {
    newSlug = validateSlug(newSlug);
    const sql = this.sql;
    const src = this.activeSourceId(sourceId);
    await sql`UPDATE pages SET slug = ${newSlug}, updated_at = now() WHERE slug = ${oldSlug} AND source_id = ${src}`;
  }
  ...
  async renamePage(oldSlug: string, newSlug: string, opts: RenamePageOpts = {}): Promise<RenamePageResult> {
    ...
    return await conn.begin(async (tx) => {
      ...
      const targetAlias = await tx`
        SELECT page_id FROM page_aliases WHERE alias_slug = ${newSlug} AND source_id = ${sourceId} LIMIT 1
      `;
      ...
      if (createAlias) {
        const aliasConflict = await tx`
          SELECT page_id FROM page_aliases WHERE alias_slug = ${oldSlug} AND source_id = ${sourceId} LIMIT 1
        `;
        if (aliasConflict.length > 0) {
          const existingAliasPageId = Number(aliasConflict[0].page_id);
          ...
          await tx`
            ...
            VALUES (${sourceId}, ${pageId}, ${oldSlug}, now())
          `;
          aliasCreated = true;
        }
      }
