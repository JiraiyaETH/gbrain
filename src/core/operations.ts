...
const put_page: Operation = {
  ...
  params: {
    ...
    no_embed: { type: 'boolean', required: false, description: 'When true, write/chunk without embedding now. Use for no-spend/no-credential writeback; embeddings can be backfilled later.' },
  },
  mutating: true,
  scope: 'write',
  handler: async (ctx, p) => {
    ...
    const noEmbed = (p.no_embed as boolean | undefined) === true || !isAvailable('embedding');
