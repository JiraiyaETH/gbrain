...
**fixed allow-list** of semantic default-source paths sourced from
`_brain-filing-rules.json`'s `dream_synthesize_paths.globs` array. Prompts
and orchestrator-owned writes use the sibling `dream_synthesize_paths.routes`
map. Editing that JSON is the ONLY way to add a new directory the synthesis
