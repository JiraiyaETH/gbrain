...
### Required context injection packet

External research MUST NOT start until the lead has created a bounded, dated `brain_context` packet. The packet is the actual input to every provider/backend lane, not merely a note that Brain was consulted:

```yaml
brain_context:
  retrieved_at: <ISO-8601>
  brain_id: <resolved brain>
  ...
  pages:
    - slug: <brain slug>
      ...
      last_known_at: <date or unknown>
  known_decisions: [<relevant decisions>]
  gaps: [<missing information>]
  contradictions: [<conflicting Brain claims>]
```

Use `search → query (if thin) → get` and record returned slugs and claims. Pass this packet verbatim (or a lossless serialized form) to the selected research backend with explicit instructions to classify findings as `new`, `changed`, `missing`, `contradictory`, or `confirming`. If the packet is empty, record `brain_context_status: empty` and do not claim novelty relative to Brain.
...
### Delta-oriented research instruction

Every external lane receives the same delta contract:

1. Find evidence relevant to the question, prioritizing new/changed/missing/contradictory information against the injected packet.
2. Preserve confirming evidence only when it establishes freshness or resolves a contradiction.
3. Return source-level provenance: URL, title, publisher, publication/access dates, retrieval route, source class, and the exact claims supported.
4. Never infer novelty from absence of a search result; label it `not found in checked sources` with the query/scope boundary.

The backend is modular. Perplexity, native web search/extract, academic APIs, browser lanes, or another approved provider may implement this contract. No provider is mandatory, and changing providers must not change the packet, ledger, claim-card, citation, or write-back gates.
...
The final packet must retain a provenance record for each included source and claim. At minimum: `source_id`, canonical URL, title, publisher/author, published date (or unknown), accessed timestamp, retrieval method, source class/authority, claim IDs supported, and caveats. A model response citation string alone is not provenance.
...
### Controlled canonical Brain write-back gate

Research output is a proposal until it passes all gates below. The lead MUST not call `put_page`, `add_timeline_entry`, `add_link`, or equivalent before the gate is explicitly recorded as passed:

```text
WRITE_BACK_GATE
  [ ] Brain context packet and target source/brain resolved
  [ ] every durable finding has claim IDs and approved source IDs
  [ ] contradictions are disclosed and dispositioned
  [ ] citation/provenance audit passed (no snippet-only support)
  [ ] finding is novel or a verified correction, not a restatement
  [ ] brain-taxonomist/schema path check passed for new pages
  [ ] operator approval exists for the specific write scope
  [ ] dry-run/rendered diff reviewed; only then use native GBrain write surface
  [ ] post-write readback + `gbrain sync --no-pull --no-embed` verified
```

For benchmark runs and unapproved research, the gate remains `not_run` and write-back is prohibited. Record proposed updates in the research artifact, never by raw filesystem writes into Brain.
...
- Skipping the Brain context packet or claiming novelty without a recorded comparison baseline.
- Treating snippets, model summaries, or uncited provider output as provenance.
- Making a backend/provider mandatory; the contract is provider-neutral.
- Writing to Brain before the controlled write-back gate passes.
- Averaging away citation, safety, contradiction, or side-effect blockers.
