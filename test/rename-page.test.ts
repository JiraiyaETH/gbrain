...
describe('page rename + alias layer', () => {
  ...
  test('atomically renames the canonical slug, creates an old-slug alias, and preserves page-owned records', async () => {
    ...
    const result = await (engine as any).renamePage(oldSlug, newSlug, {
      createAlias: true,
    ...
    expect((await engine.getChunksWithEmbeddings(newSlug)).map(c => c.chunk_text)).toEqual(['Old Dojo canonical body.']);
  ...
  test('keeps old-slug alias reads source-scoped when another source reuses the old canonical slug', async () => {
    ...
    await (engine as any).renamePage(oldSlug, newSlug, { sourceId: 'default', createAlias: true });

    await engine.putPage(oldSlug, { ...basePage, source_id: 'alt', title: 'Alt Source Dojo' });
    await engine.upsertChunks(oldSlug, [
      { chunk_index: 0, chunk_text: 'Alt source chunk.', chunk_source: 'compiled_truth' },
    ], 'alt');
    await engine.addTag(oldSlug, 'alt-tag', 'alt');
    await engine.addTimelineEntry(oldSlug, { date: '2026-05-31', summary: 'Alt event' });
    await engine.putRawData(oldSlug, 'telegram', { source: 'alt' });
    const altVersion = await engine.createVersion(oldSlug, 'alt');

    await withEnv({ GBRAIN_SOURCE: 'default' }, async () => {
      expect((await engine.getPage(oldSlug))!.slug).toBe(newSlug);
      expect(await engine.getTags(oldSlug)).toEqual(['default-tag']);
      expect((await engine.getTimeline(oldSlug)).map(e => e.summary)).toEqual(['Default event']);
      expect((await engine.getRawData(oldSlug, 'telegram'))[0].data).toEqual({ source: 'default' });
      const versionIds = (await engine.getVersions(oldSlug)).map(v => v.id);
      expect(versionIds).toContain(defaultVersion.id);
      expect(versionIds).not.toContain(altVersion.id);
      expect((await engine.getChunksWithEmbeddings(oldSlug)).map(c => c.chunk_text)).toEqual(['Default source chunk.']);
    });
  });

  test('keeps aliases source-scoped and rejects collisions without partially moving the old page', async () => {
    await engine.executeRaw(`INSERT INTO sources (id, name, config) VALUES ('alt', 'alt', '{}'::jsonb) ON CONFLICT DO NOTHING`);
    await engine.putPage('projects/dojo', { ...basePage, title: 'Default Dojo' });
    await engine.putPage('projects/dojo', { ...basePage, title: 'Alt Dojo', source_id: 'alt' });
    await engine.putPage('projects/existing', { ...basePage, title: 'Existing Target' });

    await expect((engine as any).renamePage('projects/dojo', 'projects/existing', { createAlias: true }))
      .rejects.toThrow(/already exists|collision|duplicate/i);
    expect((await engine.getPage('projects/dojo'))!.title).toBe('Default Dojo');

    await (engine as any).renamePage('projects/dojo', 'projects/default-dojo', {
      sourceId: 'default',
      createAlias: true,
    });

    expect((await engine.getPage('projects/dojo', { sourceId: 'default' }))!.slug).toBe('projects/default-dojo');
    expect((await engine.getPage('projects/dojo', { sourceId: 'alt' }))!.title).toBe('Alt Dojo');
    expect(await engine.resolveSlugs('projects/dojo', { sourceId: 'default' } as any)).toEqual(['projects/default-dojo']);
  });
});
